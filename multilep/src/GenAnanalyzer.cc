...
void GenAnalyzer::beginJob(TTree* outputTree){
    ...
    outputTree->Branch("_gen_nNu",                    &_gen_nNu,                    "_gen_nNu/b");
    ...
    outputTree->Branch("_gen_nuFlavor",               &_gen_nuFlavor,               "_gen_nuFlavor[_gen_nNu]/i");
    outputTree->Branch("_gen_nuMomPdg",               &_gen_nuMomPdg,               "_gen_nuMomPdg[_gen_nNu]/I");
    outputTree->Branch("_gen_nuIsPrompt",             &_gen_nuIsPrompt,             "_gen_nuIsPrompt[_gen_nNu]/O");
    if(multilepAnalyzer->isSUSY){
        outputTree->Branch("_gen_nNeutralino",                    &_gen_nNeutralino,                    "_gen_nNeutralino/b");
        outputTree->Branch("_gen_neutralinoPt",                   &_gen_neutralinoPt,                   "_gen_neutralinoPt[_gen_nNu]/D");
        outputTree->Branch("_gen_neutralinoEta",                  &_gen_neutralinoEta,                  "_gen_neutralinoEta[_gen_nNu]/D");
        outputTree->Branch("_gen_neutralinoPhi",                  &_gen_neutralinoPhi,                  "_gen_neutralinoPhi[_gen_nNu]/D");
        outputTree->Branch("_gen_neutralinoE",                    &_gen_neutralinoE,                    "_gen_neutralinoE[_gen_nNu]/D");
        outputTree->Branch("_gen_neutralinoMomPdg",               &_gen_neutralinoMomPdg,               "_gen_neutralinoMomPdg[_gen_nNu]/I");
        outputTree->Branch("_gen_neutralinoIsPrompt",             &_gen_neutralinoIsPrompt,             "_gen_neutralinoIsPrompt[_gen_nNu]/O");
    }
}
...
void GenAnalyzer::analyze(const edm::Event& iEvent){
    ...
    for(const reco::GenParticle& p : *genParticles){
        ...
        if(p.status() == 1 && (abs(p.pdgId()) == 12 || abs(p.pdgId()) == 14 || abs(p.pdgId()) == 16) ){
            ...
            _gen_nuPhi[_gen_nNu]      = p.phi();
            _gen_nuE[_gen_nNu]        = p.energy();
            _gen_nuIsPrompt[_gen_nNu] = p.isPromptFinalState();
            _gen_nuMomPdg[_gen_nNu]   = getMotherPdgId(p, *genParticles);
            if(abs(p.pdgId()) == 12)      _gen_nuFlavor[_gen_nNu] = 0;
            else if(abs(p.pdgId()) == 14) _gen_nuFlavor[_gen_nNu] = 1;
            else                          _gen_nuFlavor[_gen_nNu] = 2;
            ++_gen_nNu;
        }
        if(multilepAnalyzer->isSUSY){
            //store generator level neutralino info
            if(p.status() == 1 && (abs(p.pdgId()) == 1000022) ){
                if(_gen_nNeutralino == gen_nNeutralino_max) break;
                _gen_neutralinoPt[_gen_nNeutralino]       = p.pt();
                _gen_neutralinoEta[_gen_nNeutralino]      = p.eta();
                _gen_neutralinoPhi[_gen_nNeutralino]      = p.phi();
                _gen_neutralinoE[_gen_nNeutralino]        = p.energy();
                _gen_neutralinoIsPrompt[_gen_nNeutralino] = p.isPromptFinalState();
                _gen_neutralinoMomPdg[_gen_nNeutralino]   = getMotherPdgId(p, *genParticles);
                ++_gen_nNeutralino;
            }
        }
